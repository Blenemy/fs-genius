...
|---|---|---|
| `authStore` | профиль и статус сессии | любые токены, список файлов |
...
Токенов на клиенте нет: и access, и refresh — куки `httpOnly`. Вошли или нет, узнаём только через `GET /api/auth/me`.
...
- Класть токены в `localStorage` или ждать их в теле ответа
